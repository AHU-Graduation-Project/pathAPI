import { pgTable, varchar, unique, serial, boolean, timestamp, foreignKey, integer, text, doublePrecision, primaryKey, check, smallint, pgView, bigint, numeric,customType } from "drizzle-orm/pg-core"
...
export const editor = pgTable("editor", {
	...
	unique("editor_email_key").on(table.email),
]);
...
export const roadmapResource = pgTable("roadmap_resource", {
...
export const roadmap = pgTable("roadmap", {
	...
	unique("roadmap_slug_key").on(table.slug),
]);
...
export const topicResource = pgTable("topic_resource", {
	topic: integer().primaryKey().notNull(),
	title: varchar({ length: 255 }),
	link: varchar({ length: 255 }),
	type: varchar({ length: 255 }),
...
export const achievedRoadmap = pgTable("achieved_roadmap", {
	userId: integer("user_id").notNull(),
	roadmap: integer().notNull(),
}, (table) => [
	foreignKey({
	...
	primaryKey({ columns: [table.userId, table.roadmap], name: "achieved_roadmap_pkey"}),
]);

export const roadmapKeywords = pgTable("roadmap_keywords", {
	roadmap: integer().notNull(),
	...
	primaryKey({ columns: [table.roadmap, table.keyword], name: "roadmap_keywords_pkey"}),
]);

export const achievedTopic = pgTable("achieved_topic", {
	userId: integer("user_id").notNull(),
	...
	primaryKey({ columns: [table.userId, table.topic], name: "achieved_topic_pkey"}),
]);
...
export const roadmapFeedback = pgTable("roadmap_feedback", {
	...
	primaryKey({ columns: [table.roadmap, table.userId], name: "roadmap_feedback_pkey"}),
